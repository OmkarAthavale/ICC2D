...
public:
    ...
    AbstractCardiacCell* CreateCardiacCellForTissueNode(Node<2>* pNode)
    {
        ...
        ChastePoint<2> centre(0.06,0.001);
        ChastePoint<2> radii (0.01,0.01);
        ChasteEllipsoid<2> ellipseRegion(centre, radii);
        ChastePoint<2> myPoint(x, y);

        //corpus bounds
        ChastePoint<2> corpus_I_s(0.0,0.0);
        ChastePoint<2> corpus_I_e(0.12,0.31);
        ChastePoint<2> corpus_E_s(0.0,0.0);
        ChastePoint<2> corpus_E_e(0.12,0.27);
        ChastePoint<2> antrum_I_s(0.0,0.31);
        ChastePoint<2> antrum_I_e(0.12,0.36);
        ChastePoint<2> antrum_E_s(0.0,0.27);
        ChastePoint<2> antrum_E_e(0.12,0.36);

        ChasteCuboid<2> corpus_I(corpus_I_s, corpus_I_e);
        ChasteCuboid<2> corpus_E(corpus_E_s, corpus_E_e);
        ChasteCuboid<2> antrum_I(antrum_I_s, antrum_I_e);
        ChasteCuboid<2> antrum_E(antrum_E_s, antrum_E_e);
        ...
        if( setICCNode.find(index) != setICCNode.end() )
        {
            ...
            cell->SetParameter("eta", 0.038 + 0.0135 * y);

            if (corpus_I.DoesContain(myPoint))
            {
                cell->SetParameter("inhibitory_neural", 1.5);
            } else if (antrum_I.DoesContain(myPoint)) 
            {
                cell->SetParameter("inhibitory_neural", 1.8);
            } else {
                cell->SetParameter("inhibitory_neural", 1.2);                
            }

            if (corpus_E.DoesContain(myPoint))
            {
                cell->SetParameter("excitatory_neural", 0.000975);
            } else if (antrum_E.DoesContain(myPoint)) 
            {
                cell->SetParameter("excitatory_neural", 0.0016);
            } else {
                cell->SetParameter("excitatory_neural", 0.0002);                
            }
